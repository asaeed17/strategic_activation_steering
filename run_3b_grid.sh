#!/bin/bash
# Run 3B multimodel grid: 5 dims x 5 layers x 4 alphas = 100 configs

DIMS=(greed spite narcissism composure anchoring)
LAYERS=(8 12 16 20 24)
ALPHAS=(-7 5 7 15)
VECTORS_DIR=vectors/ultimatum_10dim_20pairs_general_matched/negotiation
OUTPUT_DIR=results/ultimatum/multimodel_3b

TOTAL=$(( ${#DIMS[@]} * ${#LAYERS[@]} * ${#ALPHAS[@]} ))
COUNT=0
...
for DIM in "${DIMS[@]}"; do
  for L in "${LAYERS[@]}"; do
    for A in "${ALPHAS[@]}"; do
      ...
      echo ""
      echo "=========================================="
      echo "[$COUNT/$TOTAL] 3B $DIM L${L} a=${A}"
      echo "=========================================="
      python ultimatum_game.py --model qwen2.5-3b --dimension "$DIM" \
