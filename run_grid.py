#!/usr/bin/env python3
"""
Grid runner — runs steering experiments across dimensions, layers, and alphas.
Loads model ONCE and loops through all configs sequentially.
Resume-safe: skips configs where output file already exists.

Usage:
    python run_grid.py                          # default: Qwen 7B, all 10 dims
    python run_grid.py --model llama-3.1-8b     # different model
    python run_grid.py --vectors_dir vectors/ultimatum_10dim_20pairs_general_matched/negotiation

Edit DIMENSIONS, LAYERS, ALPHAS, N_GAMES, OUTPUT_DIR below to configure the grid.
Requires: vectors extracted for the target model (see extract_vectors.py).
Hardware: Ampere+ GPU (CC >= 8.0) with native bfloat16. DO NOT use Turing/quantized.
"""
...
DIMENSIONS = [
    "firmness", "empathy", "anchoring", "composure", "fairness_norm",
    "flattery", "greed", "narcissism", "spite", "undecidedness",
]
...
def main():
    ...
    print(f"\n{'=' * 70}")
    print(f"FINAL 7B GRID — alpha=-5 ALL DIMS — {HOSTNAME}")
