#!/bin/bash
# Launch final 7B grid on scaup-l
# Dimensions: firmness, empathy, anchoring (108 configs)
set -e
...
if pgrep -f "final_grid_scaup.py" > /dev/null 2>&1; then
    echo "WARNING: final_grid_scaup.py already running. Killing old process."
    pkill -f "final_grid_scaup.py"
    sleep 2
fi

LOGFILE=/cs/student/projects1/2022/moiimran/comp0087_snlp_cwk/final_grid_scaup.log

nohup $VENV/python final_grid_scaup.py > "$LOGFILE" 2>&1 &
