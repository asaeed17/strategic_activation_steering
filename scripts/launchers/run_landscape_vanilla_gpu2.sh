#!/bin/bash
# Landscape screen - Machine: vanilla (GPU 2)
...
echo "=========================================="
echo "LANDSCAPE SCREEN - vanilla GPU 2"
echo "Start: $(date)"
...
for dim in "${DIMENSIONS[@]}"; do
  for layer in "${LAYERS[@]}"; do
    for alpha in "${ALPHAS[@]}"; do
      ...
      if [ $? -ne 0 ]; then
        echo "[${COUNT}/${TOTAL}] FAILED dim=${dim} layer=${layer} alpha=${alpha} -- $(date)"
      else
