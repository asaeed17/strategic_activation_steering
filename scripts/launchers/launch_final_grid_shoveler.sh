#!/bin/bash
# Launch final 7B grid on shoveler-l
# Dimensions: composure, flattery, spite, undecidedness (144 configs)
set -e
...
if pgrep -f "final_grid_shoveler.py" > /dev/null 2>&1; then
    echo "WARNING: final_grid_shoveler.py already running. Killing old process."
    pkill -f "final_grid_shoveler.py"
    sleep 2
fi

LOGFILE=/cs/student/projects1/2022/moiimran/comp0087_snlp_cwk/final_grid_shoveler.log

nohup $VENV/python final_grid_shoveler.py > "$LOGFILE" 2>&1 &
